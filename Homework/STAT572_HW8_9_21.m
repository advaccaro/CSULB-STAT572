%STAT572_HW8_9_21.m
%Adam Vaccaro
%Purpose: Apply some of the univariate density estimation techniques from
...
figure(1); clf;
hist(X);
title('Histogram of raw forearm data')

%% Step 3 - Find finite mixture model (nterm = 2)
p0 = [.5 .5]; %initial guess for p
mu0 = [18.5 19.5]; %initial guess for mu
...
mu0 = [17.5 18.5 19.5]; %initial guess for mu
...
[N,h] = hist(X,12);
N = N/(h(2)-h(1))/n;
bar(h,N,1,'w'); %plot histogram of forearm data
plot(xs,fhat2); %plot 2-term FM density
plot(xs,fhat3); %plot 3-term FM density
plot(xs,fhatN); %plot normal KDE
title('Histogram of forearm data with estimated densities superimposed')
legend('Foearm data','2-term FM', '3-term FM', 'Normal KDE')
axis([15 22.5 0 .4])
